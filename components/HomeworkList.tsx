...
import { GamificationService, GamificationResult } from '../services/gamificationService';
...
export const HomeworkList: React.FC<HomeworkListProps> = ({ lesson, currentUser, onUpdateLesson, onAudit }) => {
  ...
  const handleToggle = async (id: string) => {
    ...
    const targetItem = lesson.aiHomework.items.find((i) => (i.id || '') === id);
    ...
    onAudit('homework_status_changed', `${targetItem.title} を${wasCompleted ? '未完了' : '完了'}に変更`);

    // Award XP when a student completes homework
    if (nowCompleting && currentUser.role === UserRole.STUDENT) {
      try {
        // Check if completing early (1+ days before deadline)
        const meta = getHomeworkMeta(lesson.scheduledAt, targetItem);
        const isEarly = meta.daysRemaining > 0;

        const result = await GamificationService.onHomeworkComplete(currentUser.id, isEarly);
        ...
        if (result.success && result.xpGained > 0) {
          setXpAnimation({
            ...
            message: result.message || `宿題完了！ +${result.xpGained}XP`
          });
        }
      } catch (err) {
